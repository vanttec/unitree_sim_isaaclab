...
class TableStickSceneCfg(InteractiveSceneCfg):
    ...
    support_left = AssetBaseCfg(
        ...
        init_state=AssetBaseCfg.InitialStateCfg(pos=[-0.35, 0.31, 0.874],  # robot side; bottom on table (0.794)
                                                rot=[1.0, 0.0, 0.0, 0.0]),
    ...
    support_right = AssetBaseCfg(
        ...
        init_state=AssetBaseCfg.InitialStateCfg(pos=[-0.35, 0.49, 0.894],  # far side; bottom on table (0.794)
                                                rot=[1.0, 0.0, 0.0, 0.0]),
        spawn=sim_utils.CuboidCfg(
    ...
    # mid (y=0.40) line height 0.974; stick center z = 0.974 + radius(0.009) = 0.983
    # tilt ~12.5 deg about X (+Y end higher, low end toward robot) -> quat [cos6.25, sin6.25, 0, 0]
    object = RigidObjectCfg(
        prim_path="/World/envs/env_.*/Object",
        init_state=RigidObjectCfg.InitialStateCfg(pos=[-0.35, 0.40, 0.983],
                                                  rot=[0.9940, 0.1089, 0.0, 0.0]),  # tilt 12.5 deg about X
        spawn=sim_utils.CylinderCfg(
            radius=0.009,   # stick radius (~9 mm)
